...
async def upload_price_list(
    ...
    try:
        ...
        logger.info(
            f"Файл прайс-листа {file.filename} загружен от {client_host}, размер: {os.path.getsize(file_path)} байт"
        )
        ...
        processing_time = time.time() - start_time
        logger.info(
            f"Обработка прайс-листа {file.filename} завершена за {processing_time:.2f} сек., загружено {result.total_items} товаров"
        )
        ...
        logger.error(
            f"Ошибка при обработке прайс-листа {file.filename}: {str(e)}"
        )
        raise HTTPException(
            status_code=500,
...
async def search_price_list(search_query: PriceListSearchQuery):
    ...
    try:
        ...
        logger.info(
            f"Поиск по запросу '{search_query.query}' вернул {len(results)} результатов"
        )
