...
logger.add("xlsx_service.log", encoding="utf-8", rotation="10MB", 
           compression="zip",
...
class XLSXService:
    ...
    async def process_xlsx_file(self, file_path: str, original_filename: str) -> DocumentResponse:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Содержимое XLSX файла преобразовано в текст")
            ...
            logger.error(f"Ошибка при обработке XLSX файла {original_filename}: {str(e)}")
            raise
