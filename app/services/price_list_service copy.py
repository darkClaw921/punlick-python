        ...
        try:
            ...
            for idx, item in enumerate(items):
                ...
                if item_name:
                    ...
                    matches = await self.search_similar_items(query=item_name, limit=5)
                    
                    # Если найдены совпадения
                    if matches:
                        self.logger.info(f"[Товар {idx+1}] Найдено {len(matches)} потенциальных соответствий")
                        
                        best_match = None
                        best_score = 0.0
                        
                        # Логируем все найденные соответствия
                        for match_idx, match in enumerate(matches):
                            # Вычисляем сходство
                            score = self.calculate_similarity_score(item_name, match["description"])
                            
                            # Логируем информацию о каждом соответствии
                            self.logger.info(
                                f"[Товар {idx+1}][Соответствие {match_idx+1}] "
                                f"Распознано: '{item_name}' <=> Эталон: '{match['description']}' "
                                f"| Сходство: {score:.2f} ({int(score * 100)}%) "
                                f"| Категория: {match.get('category', '-')} "
                                f"| Артикул: {match.get('article', '-')}"
                            )
                        ...
                        # Если сходство выше порога, заменяем название
                        if best_match and best_score >= similarity_threshold:
                            self.logger.info(
                                f"[Товар {idx+1}] НАЙДЕНО СООТВЕТСТВИЕ! "
                                f"Распознано: '{item_name}' --> Заменено на: '{best_match['description']}' "
                                f"| Процент сходства: {best_score:.2f} ({int(best_score * 100)}%)"
                            )
                            
                            # Создаем копию данных товара
                            enriched_item = item.copy()
                            
                            # Сохраняем оригинальное название
                            original_name = enriched_item["Наименование"]
                            
                            # Обновляем данные товара
                            enriched_item["Наименование"] = best_match["name"]
                            enriched_item["Артикул"] = best_match.get("article", "")
                            enriched_item["Категория"] = best_match.get("category", "")
                            enriched_item["Подкатегория"] = best_match.get("subcategory", "")
                            enriched_item["Цена"] = best_match.get("price", 0)
                            enriched_item["Валюта"] = best_match.get("currency", "RUB")
                            enriched_item["Ед.изм."] = best_match.get("unit", enriched_item.get("Ед.изм.", ""))
                            enriched_item["Процент_совпадения"] = round(best_score * 100)
                            enriched_item["Оригинальное_название"] = original_name
                            enriched_item["matched"] = True
                            enriched_item["similarity"] = best_score
                            enriched_item["reference_id"] = best_match.get("id", "")
                            
                            # Добавляем обогащенный элемент
                            enriched_items.append(enriched_item)
                            continue
                        else:
                            if best_match:
                                self.logger.info(
                                    f"[Товар {idx+1}] Лучшее соответствие ниже порога: "
                                    f"'{best_match['name']}' с процентом {best_score:.2f} ({int(best_score * 100)}%)"
                                )
                            else:
                                self.logger.info(f"[Товар {idx+1}] Не найдено подходящих соответствий")
                    else:
                        self.logger.info(f"[Товар {idx+1}] В векторной базе не найдено потенциальных соответствий")
                else:
...
#     items = [
#     {
#         "Наименование": "Спиралка2 КР d 125 -3000 Оц.С/0,5/ [нп]",
#         "Количество": 1,
#         "Ед.изм.": "шт"
#     },
#     {
#         "Наименование": "Спиралка КР d 160 -3000 Оц.С/0,5/ [нп]",
#         "Количество": 1,
#         "Ед.изм.": "шт"
#     },
#     {
#         "Наименование": "Спиралка КР d 200 -3000 Оц.С/0,5/ [нп]",
#         "Количество": 2,
#         "Ед.изм.": "шт"
#     },
#     {
#         "Наименование": "Спиралка КР d 250 -3000 Оц.С/0,7/ [нп]",
#         "Количество": 1,
#         "Ед.изм.": "шт"
#     }
# ]
#     price_list_service = PriceListService()
#     asyncio.run(price_list_service.find_matching_items(items))
