...
logger.add("ocr_service.log", encoding="utf-8", rotation="10MB", 
           compression="zip",
...
class OCRService:
    def __init__(self):
        self._client = Mistral(api_key=settings.MISTRAL_API_KEY, timeout_ms=300000)# 5 минут
    ...
    def send_mistral_document_batch(self, pages):
        all_text=[]
        count_pages=0
        for page in tqdm(pages, desc="Обработка страниц из документа (Mistral API)"):
            if count_pages >= 20:
                return all_text
    ...
    async def process_document(self, file_path: str, original_filename: str, file_type: str = None) -> DocumentResponse:
        ...
        document_response = DocumentResponse(
            ...
            items=[DocumentItem(text=item if isinstance(item, str) else json.dumps(item, ensure_ascii=False)) 
    ...
    async def process_image(self, file_path: str, original_filename: str) -> DocumentResponse:
        ...
        try:
            ...
            promt=open("app/services/Pasted--1--1741736569399.txt", "r").read()
            messages = [
                {
                    "role": "user",
                    "content": [
            ...
            messages = [
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": promt + "и верни их в виде списка в формате json Наименование|Количество|Ед.изм."
                        },
                    ]
                },
                {
            ...
            logger.error(f"Ошибка при обработке изображения {original_filename}: {str(e)}")
            raise
