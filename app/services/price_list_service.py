...
from app.models.document import PriceListResponse, PriceListItem, PriceListUpload
...
class PriceListService:
    ...
    def __init__(self):
        ...
        logger.add(self.log_file, encoding="utf-8", rotation="10MB", 
                  compression="zip", format="{time}|{file}:{line}|{level} {message}", level="INFO")
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Коллекция {settings.CHROMA_COLLECTION_NAME} успешно подключена")
            except:
    ...
    async def process_price_list(self, file_path: str, original_filename: str) -> PriceListResponse:
        """
        ...
        try:
            ...
            self.logger.info(f"Прайс-лист {original_filename} успешно обработан, " 
                            f"загружено {total_items} товаров из {categories_count} категорий")
            
            return response
            
        except Exception as e:
            self.logger.error(f"Ошибка при обработке прайс-листа {original_filename}: {str(e)}")
    ...
    def _read_csv_price_list(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            required_columns = ['category', 'subcategory', 'article', 'name', 'price', 'unit']
            ...
            price_list_data = {
                'price_list_date': df.get('price_list_date', [None])[0] or "2023-01-01",
    ...
    def _read_json_price_list(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'price_list_date' not in data:
                raise ValueError("В JSON файле отсутствует обязательное поле: price_list_date")
            
            if 'currency' not in data:
                raise ValueError("В JSON файле отсутствует обязательное поле: currency")
            
            if 'categories' not in data:
                raise ValueError("В JSON файле отсутствует обязательное поле: categories")
    ...
    def _load_data_to_chroma(self, data: Dict[str, Any], price_list_id: str) -> int:
        """
        ...
        try:
            ...
            self.logger.info(f"В ChromaDB успешно загружено {total_items} товаров из прайс-листа {price_list_id}")
            return total_items
            
        except Exception as e:
            self.logger.error(f"Ошибка при загрузке данных в ChromaDB: {str(e)}")
            raise Exception(f"Ошибка при загрузке данных в ChromaDB: {str(e)}")
    
    async def search_similar_items(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.info(f"Поиск по запросу '{query}' вернул {len(items)} результатов")
            return items
